...
let layoutManager = {
    ...
    showBlock: function(item, setting) {
        ...
        let time = typeof setting.animate == 'string' || typeof setting.animate == 'number' ? setting.animate : 0.5
        let block = runtime.blocks[item.id];
        let $item = $(item);
        let self = this;
        let start = $item.attr("data-state") != "move";
        let trans = start ? "width "+time+"s, height "+time+"s": "top "+time+"s, left "+time+"s, width "+time+"s, height "+time+"s, opacity "+time+"s";
...
function Freewall(selector) {
    ...
    $.extend(klass, {
        ...
        */
        ...
        refresh: function() {
            let params = arguments.length ? arguments : runtime.arguments;
            let oldArg = runtime.arguments;
            let method = oldArg ? oldArg.callee : this.fitWidth;
